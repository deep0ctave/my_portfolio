...
export function App() {
  // Live controls for the grid — use the Leva panel in the running app to tweak.
  const { size, divisions, colorCenter, colorLines, visible, posX, rotZ } = useControls('Grid', {
    size: { value: 10, min: 0, max: 50, step: 1 },
    divisions: { value: 40, min: 1, max: 200, step: 1 },
    colorCenter: { value: '#101010' },
    colorLines: { value: '#050505' },
    visible: true,
    posX: { value: -0.25, min: -5, max: 5, step: 0.01 },
    rotZ: { value: Math.PI / 2, min: 0, max: Math.PI * 2, step: 0.01 }
  })
  ...
  return (
    <Canvas orthographic dpr={[1, 2]} camera={{ position: [0, 0, 10], zoom: 200 }}>
      <group rotation={[Math.PI / 5, -Math.PI / 5, Math.PI / 2]}>
        ...
        <gridHelper
          args={[size, divisions, colorCenter, colorLines]}
          position={[posX, 0, 0]}
          rotation={[0, 0, rotZ]}
          visible={visible}
        />
